...
## Project status
No implementation exists yet. `README.md` is **both** the authoritative spec and the living project doc — the user wants all project details kept there, not in new doc files. Spec target: a Python 3.12 / Docker, IMAP-only email organizer.
...
2. **Implement.**
3. **Update `CHANGELOG.md`** with a `X.X.X` semantic-version entry.
4. **Commit** with a good subject + body (do NOT repeat the vague existing history like "commit"/"doc").
5. **Push to `dev-001`**, referencing the `X.X.X` version.
...
## Hard technical constraints (from README spec — do not violate)
...
The user's actual goal is to **log in to the mailbox and group emails by sender's domain, recursively**. The README's flat `LABEL_RULES` scheme is the baseline; satisfy the recursive domain-grouping intent. Record implementation details in `README.md`, not here.

## Verification
No test suite, lint, or typecheck is configured yet. Use the README's ACCEPTANCE CHECKLIST as the done-definition:
